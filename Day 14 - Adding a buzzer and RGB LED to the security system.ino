...
int redPin = 11;  // PWM colour output pins
int greenPin = 10;
int bluePin = 9;
...
Keypad securityPad = Keypad(makeKeymap(buttons), rowPins, colPins, ROWS, COLS); // This maps the characters for our buttons onto the relevant pins of the HERO
...
void loop() {
  ...
  if(result == '*'){  // choice to change password
    ...
    if(access < 0) {
      ...
      RGB_color(1, 125, 0); // LED to GREEN
    }
  }
